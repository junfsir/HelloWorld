...
注释表明，`syncLoop`函数是`kubelet`的主循环；该函数通过监听`updates`来获取最新的`Pod`配置，并同步`Pod`的运行状态和期望状态；通过这种方式，所有运行在该`node`上的`Pod`都将运行在期望的状态；事实上，`syncLoop`只封装了`syncLoopIteration`，同步操作由`syncLoopIteration`来执行。
