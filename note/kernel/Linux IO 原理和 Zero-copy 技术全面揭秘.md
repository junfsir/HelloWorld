...
![img](https://img.taohuawu.club/gallery/typical-memory-hierarchy.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/swapping-memory.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/virtual-memory-pages.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/accessing-memory-by-mmu.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/virtual-memory-translation.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/slide_13.jpg?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/kernel-space-and-user-space.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

## Linux I/O

### I/O 缓冲区

![img](https://img.taohuawu.club/gallery/summary-of-io-buffering.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/buffer-cache-page-cache.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/buffer-cache-in-page-cache.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/blocking-io-flow.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)

#### 中断驱动 I/O

第二种 I/O 模式是利用中断来实现的：

![img](https://img.taohuawu.club/gallery/linux-os-io-interrupts.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/linux-os-io-dma.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/Linux-io-read-write.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
零拷贝技术是指计算机执行操作时，[CPU](https://zh.wikipedia.org/wiki/中央处理器)不需要先将数据从某处[内存](https://zh.wikipedia.org/wiki/随机存取存储器)复制到另一个特定区域。这种技术通常用于通过网络传输文件时节省CPU周期和[内存带宽](https://zh.wikipedia.org/wiki/内存带宽)。
...
![img](https://img.taohuawu.club/gallery/linux-zero-copy-stack.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
2
3void *mmap(void *addr, size_t length, int prot, int flags, int fd, off_t offset);
...
![img](https://img.taohuawu.club/gallery/Linux-io-mmap-write.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
2
3ssize_t sendfile(int out_fd, int in_fd, off_t *offset, size_t count);
```
...
![img](https://img.taohuawu.club/gallery/Linux-io-sendfile.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/Linux-io-sendfile-dma-gather.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
2#include <unistd.h>
3
4int pipe(int pipefd[2]);
5int pipe2(int pipefd[2], int flags);
6
7ssize_t splice(int fd_in, loff_t *off_in, int fd_out, loff_t *off_out, size_t len, unsigned int flags);
```
...
3pipe(pfd);
4
5ssize_t bytes = splice(file_fd, NULL, pfd[1], NULL, 4096, SPLICE_F_MOVE);
6assert(bytes != -1);
7
8bytes = splice(pfd[0], NULL, socket_fd, NULL, bytes, SPLICE_F_MOVE | SPLICE_F_MORE);
9assert(bytes != -1);
```

数据传输过程图：

![img](https://img.taohuawu.club/gallery/linux-io-splice.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/pipe-buffer-underlying-impl.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
splice()` 写入数据到管道的调用链式：`do_splice()` --> `do_splice_to()` --> `splice_read()
 1static long do_splice(struct file *in, loff_t __user *off_in,
 2		      struct file *out, loff_t __user *off_out,
 3		      size_t len, unsigned int flags)
 4{
 5...
 6
 7	// 判断是写出 fd 是一个管道设备，则进入数据写入的逻辑
 8	if (opipe) {
 9		if (off_out)
10			return -ESPIPE;
11		if (off_in) {
12			if (!(in->f_mode & FMODE_PREAD))
13				return -EINVAL;
14			if (copy_from_user(&offset, off_in, sizeof(loff_t)))
15				return -EFAULT;
16		} else {
17			offset = in->f_pos;
18		}
19
20		// 调用 do_splice_to 把文件内容写入管道
21		ret = do_splice_to(in, &offset, opipe, len, flags);
22
23		if (!off_in)
24			in->f_pos = offset;
25		else if (copy_to_user(off_in, &offset, sizeof(loff_t)))
26			ret = -EFAULT;
27
28		return ret;
29	}
...
```c
 1static long do_splice_to(struct file *in, loff_t *ppos,
 2			 struct pipe_inode_info *pipe, size_t len,
 3			 unsigned int flags)
 4{
 5	ssize_t (*splice_read)(struct file *, loff_t *,
 6			       struct pipe_inode_info *, size_t, unsigned int);
 7	int ret;
 8
 9	if (unlikely(!(in->f_mode & FMODE_READ)))
10		return -EBADF;
11
12	ret = rw_verify_area(READ, in, ppos, len);
13	if (unlikely(ret < 0))
14		return ret;
15
16	if (unlikely(len > MAX_RW_COUNT))
17		len = MAX_RW_COUNT;
18
19	// 判断文件的文件的 file 结构体的 f_op 中有没有可供使用的、支持 splice 的 splice_read 函数指针
20	// 因为是 splice() 调用，因此内核会提前给这个函数指针指派一个可用的函数
21	if (in->f_op->splice_read)
22		splice_read = in->f_op->splice_read;
23	else
24		splice_read = default_file_splice_read;
25
26	return splice_read(in, ppos, pipe, len, flags);
27}
```
...
```c
 1ssize_t splice_to_pipe(struct pipe_inode_info *pipe, struct splice_pipe_desc *spd)
 2{
 3...
 4
 5	for (;;) {
 6		if (!pipe->readers) {
 7			send_sig(SIGPIPE, current, 0);
 8			if (!ret)
 9				ret = -EPIPE;
10			break;
11		}
12
13		if (pipe->nrbufs < pipe->buffers) {
14			int newbuf = (pipe->curbuf + pipe->nrbufs) & (pipe->buffers - 1);
15			struct pipe_buffer *buf = pipe->bufs + newbuf;
16
17			// 写入数据到管道，没有真正拷贝数据，而是内存地址指针的移动，
18			// 把物理页框、偏移量和数据长度赋值给 pipe_buffer 完成数据入队操作
19			buf->page = spd->pages[page_nr];
20			buf->offset = spd->partial[page_nr].offset;
21			buf->len = spd->partial[page_nr].len;
22			buf->private = spd->partial[page_nr].private;
23			buf->ops = spd->ops;
24			if (spd->flags & SPLICE_F_GIFT)
25				buf->flags |= PIPE_BUF_FLAG_GIFT;
26
27			pipe->nrbufs++;
28			page_nr++;
29			ret += buf->len;
...
34			if (!--spd->nr_pages)
35				break;
36			if (pipe->nrbufs < pipe->buffers)
37				continue;
38
39			break;
40		}
...
```c
1if (setsockopt(socket_fd, SOL_SOCKET, SO_ZEROCOPY, &one, sizeof(one)))
2        error(1, errno, "setsockopt zerocopy");
3
4ret = send(socket_fd, buffer, sizeof(buffer), MSG_ZEROCOPY);
```
...
```c
 1pfd.fd = fd;
 2pfd.events = 0;
 3if (poll(&pfd, 1, -1) != 1 || pfd.revents & POLLERR == 0)
 4        error(1, errno, "poll");
 5
 6ret = recvmsg(fd, &msg, MSG_ERRQUEUE);
 7if (ret == -1)
 8        error(1, errno, "recvmsg");
 9
10read_notification(msg);
11
12
13uint32_t read_notification(struct msghdr *msg)
14{
15	struct sock_extended_err *serr;
16	struct cmsghdr *cm;
17	
18	cm = CMSG_FIRSTHDR(msg);
19	if (cm->cmsg_level != SOL_IP &&
20		cm->cmsg_type != IP_RECVERR)
21			error(1, 0, "cmsg");
22	
23	serr = (void *) CMSG_DATA(cm);
24	if (serr->ee_errno != 0 ||
25		serr->ee_origin != SO_EE_ORIGIN_ZEROCOPY)
26			error(1, 0, "serr");
27	
28	return serr->ee _ data;
29}
```
...
![img](https://img.taohuawu.club/gallery/linux-direct-io-by-user.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
...
![img](https://img.taohuawu.club/gallery/linux-io-buffer-sharing-fbufs.png?imageView2/2/w/1280/format/jpg/interlace/1/q/100)
